...
void main() {
  group('ThullaEngine', () {
    test(
      'Initialization deals correct number of cards and sets starting player',
      () {
        final players = [
    ...
    test('Validates first trick requires Ace of Spades', () {
      ...
      final wrongCard = startPlayer.hand.firstWhere(
        (c) => c != const PlayingCard(suit: Suit.spades, rank: Rank.ace),
      );
      ...
      final correctError = ThullaEngine.getMoveError(
        state,
        startPlayerId,
        const PlayingCard(suit: Suit.spades, rank: Rank.ace),
      );
