...
void main() {
  group('BluffEngine Tests', () {
    ...
    test('Playing 1 card on empty pile throws error', () {
      ...
      expect(() => BluffEngine.playCards(state, 'p1', cardsToPlay, Rank.ace), throwsException);
    });

    test('Subsequent player can play 1 card', () {
      final player1 = state.players[0];
      var s1 = BluffEngine.playCards(state, 'p1', player1.hand.take(2).toList(), Rank.ten);
      
      final player2 = s1.players[1];
      var s2 = BluffEngine.playCards(s1, 'p2', player2.hand.take(1).toList(), Rank.jack);
