...
void main() {
  group('ThullaEngine', () {
    test('Initialization deals 52 cards and gives Ace of Spades to start', () {
      ...
      final starter = state.players.firstWhere(
        (p) => p.id == state.currentPlayerId,
      );
      expect(
        starter.hand.contains(
      ...
      state = state.copyWith(
        players: [
          ...
          Player(
            id: 'p2',
            name: 'p2',
            hand: const [PlayingCard(suit: Suit.diamonds, rank: Rank.two)],
          ), // No hearts!
          Player(
            id: 'p3',
            name: 'p3',
    ...
    test('End Game Win Condition triggers when 1 player left', () {
      ...
      state = ThullaEngine.resolveTrick(
        state.copyWith(
          trickResolving: true,
          currentTrick: [
            TrickPlay(
              playerId: 'p3',
              card: const PlayingCard(suit: Suit.hearts, rank: Rank.two),
            ),
          ],
        ),
      );
